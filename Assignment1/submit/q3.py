...
class LogisticGrowthModel(dynamical_model.DynamicalModel):
...
if __name__ == "__main__":
    ...
    for j in range(len(methods)):
        for i in range(len(h)):
            ...
            df = model.get_data()
            plt.plot(df.iloc[:,0], df.iloc[:,1], dynamical_model.constant.line_styles[j],color=dynamical_model.constant.colors[j], label=methods[j])
            plt.xlabel("Time")
